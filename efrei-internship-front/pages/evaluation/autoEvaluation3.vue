<template>
  <div>
    ...
    <form @submit.prevent="sendEvaluation">
      <label>
        De manière générale, comment vos missions ont-elles évolué ? Expliquer comment s’est effectuée la transition entre
      ...
      <label>
        Quelles ont été vos meilleures réalisations pendant votre stage ?
        Donnez des exemples concrets.
        <textarea name="input2" v-model="input2" required></textarea>
      </label>
      <label>
        Quels apprentissages pouvez-vous en tirer ?
        (Sur vos goûts, vos talents, vos limites, vos souhaits…)
        <textarea name="input3" v-model="input3" required></textarea>
      </label>
      <label>
        Quelles compétences avez-vous développé pour mener à bien vos missions ? Comment les avez-vous utilisées ?
      ...
      <label>
        Quel bilan faites-vous de votre projet professionnel (objectif professionnel, idée(s) de métier, orientations
